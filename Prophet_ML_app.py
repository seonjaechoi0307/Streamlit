...
def Model_data_Visualization():
    ...
    ax.set_xlabel('날짜', fontsize=48)
    ax.set_ylabel('예측 가격', fontsize=48)
    ax.set_title('월간 가격 예측 그래프', fontsize=48)
    ...
    ax.plot(future_prices_df['Date'], future_prices_df['ma_4'], linewidth=4, linestyle='--', color='green', label='ma_120')
    ax.plot(future_prices_df['Date'], future_prices_df['ma_8'], linewidth=4, linestyle='--', color='orange', label='ma_240')
    ax.plot(future_prices_df['Date'], future_prices_df['ma_12'], linewidth=4, linestyle='--', color='purple', label='ma_360')
